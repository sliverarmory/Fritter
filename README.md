...
The package accepts payload bytes through domain-specific request structs, uses typed native DLL and .NET entrypoints plus loader and staging configuration, and returns generated artifacts as bytes. It deliberately does not expose target argv, raw command lines, or target parameters. The canonical WASM module is embedded, so importing applications do not need a sidecar file, C toolchain, or CGO. See [`GO.md`](./GO.md) for the complete API.
